#!/usr/bin/env python3
"""
Parallel version of the case data cleanup script.
Uses multiprocessing to process multiple files simultaneously for faster execution.
"""
...
def clean_json_file(filepath: Path, rename: bool = True) -> Tuple[bool, str]:
    ...
    try:
        ...
        if modified:
            ...
            return True, f"unchanged:{filepath.name}"
    
    except json.JSONDecodeError as e:
        return False, f"error:{filepath.name}:JSON decode error"
...
def process_directory_parallel(directory: Path, rename: bool = True, num_workers: Optional[int] = None) -> Dict[str, int]:
    """
    Process all JSON files in directory recursively using parallel processing.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Clean and standardize case JSON files (parallel version)'
    )
    parser.add_argument(
        'directory',
        type=str,
    ...
    stats = process_directory_parallel(
        directory,
