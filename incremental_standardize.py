    ...
    try:
        ...
        if not gr_number or "gr_l-" in gr_number.lower():
            ...
            if not gr_number or "gr_l-" in gr_number.lower():
                ...
                if match: gr_number = match.group(2)
            if gr_number:
        ...
        if "division" not in data: data["division"] = "En Banc"; modified = True
        if "categories" not in data: data["categories"] = []; modified = True
        if "keywords" not in data: data["keywords"] = []; modified = True
        if "formatted_case_content" not in data: data["formatted_case_content"] = ""; modified = True
        if "decision_date" not in data: data["decision_date"] = f"{year}-01-01"; modified = True

        clean_gr = clean_gr_number_for_filename(data.get("gr_number", ""))
        if not clean_gr: clean_gr = f"unknown_{os.path.basename(filepath)}"
